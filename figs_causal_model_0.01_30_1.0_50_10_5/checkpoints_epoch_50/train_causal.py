...
# Import Causal Inference Model
from src.tfmodel import CausalInferenceModel  # Ensure CausalInferenceModel is defined in tfmodel
...
signature = 'causal_model_'  # Updated signature for causal model
...
for epoch in range(start_epoch, epochs + 1):
    ...
    for i in range(ROUNDS):
        ...
        o0, o1, pi0, log_Ppi = u.make_batch_dsprites_active_inference(
            games=games,
            model=model.active_inf_model,  # Use the active inference model here
            deepness=deepness,
            samples=samples,
            calc_mean=True,
            repeats=repeats
    ...
    if epoch % 25 == 0:
        ...
        # Remove optimizer checkpoints if not needed
        # os.remove(f"{folder_chp}_epoch_{epoch}/optimizers.pkl")  # Uncomment if needed
    ...
    generate_traversals(
        model=model.active_inf_model,  # Use the active inference model
        s_dim=s_dim,
        s_sample=s0,
        S_real=S0_real,
    ...
    reconstructions_plot(o0, o1, po1.numpy(), filename=f"{folder}/imagination_{signature}_{epoch}.png", colour=False)

    # Test how well the agent learnt the dynamics related to the reward
    o0, o1, pi0 = u.make_batch_dsprites_random_reward_transitions(
        game=game_test,
        index=0,
        size=TEST_SIZE,
        repeats=repeats
    ...
    reconstructions_plot(o0, o1, po1.numpy(), filename=f"{folder}/reward_imagination_{signature}_{epoch}.png")
    ...
    print(f"{epoch}, F: {stats['F'][-1]:.2f}, MSEo: {stats['mse_o'][-1]:.3f}, KLs: {stats['kl_div_s'][-1]:.2f}, "
